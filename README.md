...
| 同上 · 最快一次 | 7,835 s · 16 kHz · mono · PCM16 | 3.90 s | **≈ 2,010×** |
...
- **动态 VAD** — 可选的动态静音阈值（`--dynamic-vad`），语音段越长静音切分
  越紧，保长段、减少边界切割损失（来自 FunASR offline 策略）。
...
`VadConfig.silence_schedule` 也可编程设置：传入 `FUNASR_OFFLINE_SCHEDULE.to_vec()`
（或你自己的 `(upper_ms, threshold_ms)` 表）即可从库代码启用动态静音切分 ——
效果与 CLI 的 `--dynamic-vad` 开关相同。
...
| 参数 | 默认值 | 说明 |
|:--|:--|:--|
| `<wav_path>` | *(必填)* | 输入 WAV 路径 |
| `--speech-threshold`    | `0.4`   | 语音概率阈值 |
| `--smooth-window-size`  | `5`     | 滑动窗口平滑（帧） |
| `--min-speech-frame`    | `20`    | 确认为语音所需的最少连续帧 |
| `--max-speech-frame`    | `2000`  | 单段语音的最大长度（超过则强制切分） |
| `--min-silence-frame`   | `20`    | 确认为静音所需的最少连续帧 |
| `--merge-silence-frame` | `0`     | 短于该帧数的静音缝直接合并 |
| `--extend-speech-frame` | `0`     | 语音段前后各延展的帧数 |
| `--chunk-max-frame`     | `30000` | 并行分块推理的最大帧数 |
| `--dynamic-vad`         | `off`   | 启用动态静音阈值（FunASR offline 策略，段越长静音切分越紧） |
